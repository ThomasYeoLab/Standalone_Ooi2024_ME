function CBIG_ME_ABCD_KRR_randomFC(num_sites, innerFolds, mins, vers, ...
...
% of single-kernel regression. This function only runs for a specific subsample.
% 
% Inputs:
%   - num_sites
...
for k = 1:length(sub_fold)
    ...
    if ~exist(fullfile(param.outdir,strcat('final_result_',outstem,'.mat')))
        ...
        CBIG_KRR_workflow_LITE(param);
         ...
         fprintf('Final result file exists, skipping... \n')
    end
end
...
for j = 1:length(sub_samples)
    ...
    for k = 1:length(sub_fold)
        ...
        % run KRR workflow for subsample
        if ~exist(fullfile(param.outdir,strcat('final_result_',outstem,'.mat')))
            fprintf('Collate feature matrices... \n')
            % read subject list
            f = fopen(sub_txt);
