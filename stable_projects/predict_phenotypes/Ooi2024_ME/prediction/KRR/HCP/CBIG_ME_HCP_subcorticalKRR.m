function CBIG_ME_HCP_subcorticalKRR(outerFolds, innerFolds, seed, mins, vers, ...
...
% of single-kernel regression. The input feature is transformed to use the subcortical 
% portion 419 x 19 FC to predict the phenotype.
% 
% Inputs:
%   - outerFolds
...
outdir = fullfile(basedir,'output', strcat(vers,'_sc'));
...
% generate folds
if ~exist(fullfile(outdir, seed_name, 'no_relative_10_fold_sub_list.mat'))
    ...
    fold_temp = load(fullfile(outdir, seed_name, 'no_relative_10_fold_sub_list.mat'));
...
if strcmp(vers,'random')
    ...
    if ~exist(FC_mat)
        ...
        if contains(vers, '1000parcels')
            feat_mat = zeros(1017,1017,length(subj_list));
        else
            feat_mat = zeros(419,419,length(subj_list));
        end
        % read files
...
%% extract subcortical features only
% pre allocate space
sc_feat_mat = zeros(19*419, size(feat_mat,3));
% read files
for s = 1:size(feat_mat,3)
    sub_corrmat = feat_mat(401:419,:,s);
    sc_feat_mat(:,s) = sub_corrmat(:);
end
...
sub_samples = [100 120 150 200 300 400 500 600];
for j = 1:length(sub_samples)
    ...
    for k = 1:length(sub_fold)
        ...
        for col = 1:size(param.y, 2)
            ...
            if numel(unique(param.y(:,col)))==1
                fprintf('\t WARNING: var %i is only has 1 outcome. Please ignore for downstream analysis. \n', col)
                rng(seed,'twister')
                param.y(:, col) = param.y(:, col) + rand(length(fold_pos),1) * 1e-6;
            end
        end
        param.covariates = cov(fold_pos, :);
        param.feature_mat = sc_feat_mat(:,fold_pos);
        param.outdir = fullfile(outdir, seed_name, outstem, ...
