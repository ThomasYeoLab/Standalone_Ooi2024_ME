function CBIG_ME_calc_ABCD_FC(subj_txt, mins, vers, perm_order, batch, res)

% function CBIG_ME_calc_ABCD_FC(subj_txt, mins, vers, perm_order, batch)
%
...
for n = 1:length(final_list)
    ...
    if ~exist(output_file)
        ...
        for i = 1:4 % corresponds to the 4 runs in ABCD
            run = num2str(run_order(i));
            lh_file = fullfile(indiv_path, 'surf', strcat('lh.', sub, '_bld00', ...
                run,'_rest_mc_skip_residc_interp_FDRMS0.3_DVARS50_bp_0.009_0.08_fs6_sm6.nii.gz'));
            rh_file = fullfile(indiv_path, 'surf', strcat('rh.', sub, '_bld00', ...
                run,'_rest_mc_skip_residc_interp_FDRMS0.3_DVARS50_bp_0.009_0.08_fs6_sm6.nii.gz'));
            bold_file = fullfile(indiv_path, 'bold', strcat('00',run), ...
                strcat(sub, '_bld00', run,'_rest_mc_skip_residc_interp_FDRMS0.3_DVARS50_bp_0.009_0.08.nii.gz'));
            cens_txt = fullfile(indiv_path, 'qc', ...
                strcat(sub, '_bld00',run,'_FDRMS0.3_DVARS50_motion_outliers.txt'));
