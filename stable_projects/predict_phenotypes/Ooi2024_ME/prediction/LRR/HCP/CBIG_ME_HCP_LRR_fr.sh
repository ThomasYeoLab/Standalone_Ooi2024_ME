...
#####
# This wrapper script submits job to the schduler to run LRR in the HCP dataset.
# This runs the prediction procedure (10-fold cross validation).
#
# Input:
#    -min:
...
scriptdir=$ME_CODE_DIR/prediction/LRR/HCP
...
mkdir -p $projectdir/output/$vers/seed_${seed}/logs
LF="$projectdir/output/$vers/seed_${seed}/logs/${outstem}_${seed}.log"
...
matlab -nodesktop -nosplash -nodisplay -r " try addpath('$scriptdir'); CBIG_ME_HCP_LRR_fr($outerFolds, \
   $innerFolds, $seed, $min, '$vers', '$projectdir', '$subtxt', '$scorecsv', \
