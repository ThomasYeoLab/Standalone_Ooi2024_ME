...
#
#    -ss:
#     An integer representing the subsample size.
#
# EXAMPLE: 
#    CBIG_ME_ABCD_KRR_splithalf_subsample.sh 20 full
#
...
LF="$projectdir/output_splithalf/$vers/logs/${outstem}_ss${ss}.log"
...
matlab -nodesktop -nosplash -nodisplay -r " try addpath('$scriptdir'); CBIG_ME_ABCD_KRR_randomFC_splithalf_subsample( \
$num_sites, $innerFolds, $min, '$vers', '$projectdir', '$subtxt', '$subcsv', \
'$predvar', '$covtxt', '$ymat','$covmat', $ss); catch ME; display(ME.message); \
end; exit; " >> $LF 2>&1
