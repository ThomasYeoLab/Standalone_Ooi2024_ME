...
#
# Example:
# ssh headnode; ${TANNGUYEN2025_BA_DIR}/replication/scripts/ad_classification/CBIG_BA_direct.sh \
# -d "train" -s "0 1 2 3 4" -z "448";
...
CONFIG_CSV="${TANNGUYEN2025_BA_DIR}/data/params/${task}/${model}.csv"  # Hyperparameters configuration
...
if [[ $mode == "train" ]]; then 
    ...
    cmd+="python -m model.cnn.CBIG_BA_test --data_split ${split_list}" 
    cmd+=" --model_dir ${OUT_DIR} --test_csv ${TEST_CSV} --task ${task}"
    cmd+=";conda deactivate"
