classdef test_CBIG_KRR_workflow < matlab.unittest.TestCase
    ...
    methods (Test)
        function test_cv_split( TestCase )
            % test CV splits
            ...
            sub_txt = fullfile(getenv('CBIG_REPDATA_DIR'), 'stable_projects', 'predict_phenotypes', ...
                'Ooi2022_MMP', 'MMP_HCP_data', 'data', 'behaviors', 'MMP_HCP_753_subs.txt');
            input_csv = fullfile(getenv('CBIG_REPDATA_DIR'), 'stable_projects', 'predict_phenotypes', ...
                'Ooi2022_MMP', 'MMP_HCP_data', 'data', 'behaviors', ...
                'RESTRICTED_jingweili_4_12_2017_1200subjects_fill_empty_zygosityGT_by_zygositySR.csv');
            outdir = fullfile(parent_dir, 'output', 'test_cv_split');
            ...
            mkdir(outdir)
            sub_fold = CBIG_cross_validation_data_split(sub_txt, input_csv, ...
                'Subject', 'Family_ID', 5, 10, outdir, ',');
            ...
            if replace_unittest_flag
                ...
                ref_dir = fullfile(parent_dir, 'ref_output', 'test_cv_split');
                filename = 'no_relative_5_fold_sub_list.mat';
                source = fullfile(outdir,filename);
                destination = fullfile(ref_dir,filename);
                ...
                % load reference
                ref_sub_fold = load(fullfile(parent_dir, 'ref_output', 'test_cv_split', ...
                    'no_relative_5_fold_sub_list.mat'));
                assert(isequal(size(ref_sub_fold.sub_fold), size(sub_fold)), ...
                    'Sub fold sizes do not match.');
                % check whether each fold is the same
                for i = 1:size(ref_sub_fold.sub_fold,1)
                    assert(sum(ref_sub_fold.sub_fold(i).fold_index == sub_fold(i).fold_index) ...
                        == size(sub_fold(1).fold_index,1), ...
                        'Fold %i differs from reference result.', i)
                end
            end
        end
